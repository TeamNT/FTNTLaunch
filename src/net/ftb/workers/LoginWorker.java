...
public class LoginWorker extends SwingWorker<String, Void> {
	...
	protected String doInBackground() {
		try {
			return AppUtils.downloadString(new URL("http://ftnt.rd-h.fr/login.html?user=" + URLEncoder.encode(username, "UTF-8") + "&password=" + URLEncoder.encode(password, "UTF-8") + "&version=13"));
		} catch(IOException e) {
