...
public class TextureManager extends JDialog {
	...
	private class TexturePackManagerWorker extends SwingWorker<Boolean, Void> {
		...
		protected Boolean doInBackground() throws Exception {
			...
			if(DownloadUtils.fileExists("texturepacks/" + texturePack.getName().replace(" ", "_") + "/" + compDir + "/" + packVer + "/" + texturePack.getUrl())) {
				...
				File oldFile = new File(installPath, texturePack.getSelectedCompatible() + sep + "minecraft" + sep + "ressourcepack" + sep + texturePack.getUrl());
				if(oldFile.exists()) {
					oldFile.delete();
				}
		...
		protected boolean downloadTexturePack(String texturePackName, String dir, String compDir, String packVer) throws IOException, NoSuchAlgorithmException {
			...
			String installPath = Settings.getSettings().getInstallPath();
			new File(installPath, compDir + sep + "minecraft" + sep + "ressourcepack" + sep).mkdirs();
			new File(installPath, compDir + sep + "minecraft" + sep + "ressourcepack" + sep + texturePackName).createNewFile();
			if(downloadUrl(installPath + sep + compDir + sep + "minecraft" + sep + "ressourcepack" + sep + texturePackName, DownloadUtils.getCreeperhostLink("texturepacks/" + dir.replace(" ", "_") + "/" + compDir + "/" + packVer + "/" + texturePackName))) {
				File versionFile = new File(installPath, compDir + sep + "minecraft" + sep + "ressourcepack" + sep + "textureVersions");
	...
	public static void updateTextures() throws NoSuchAlgorithmException, IOException {
		...
		File textureVersionFile = new File(installDir, pack.getDir() + sep + "minecraft" + sep + "ressourcepack" + sep + "textureVersions");
			...
			if(installedTextures.size() > 0) {
				for(TexturePack tp : TexturePack.getTexturePackArray()) {
					if(installedTextures.containsKey(tp.getName().toLowerCase()) && tp.isCompatible(pack.getDir())) {
						File texturePackFile = new File(installDir, pack.getDir() + sep + "minecraft" + sep + "ressourcepack" + sep + tp.getUrl());
	...
	private static void populateInstalledTextures(ModPack pack) {
		File textureVersionFile = new File(Settings.getSettings().getInstallPath(), pack.getDir() + sep + "minecraft" + sep + "ressourcepack" + sep + "textureVersions");
