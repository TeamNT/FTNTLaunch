...
public class AuthlibHelper
{
	...
	protected static LoginResponse authenticateWithAuthlib (String user, String pass, String mojangData, String selectedProfileName)
	{
		...
		YggdrasilUserAuthentication authentication = (YggdrasilUserAuthentication)new YggdrasilAuthenticationService(proxy, OSUtils.getClientToken().toString()).createUserAuthentication(Agent.MINECRAFT);
		if (user != null)
		{
			...
			if (authentication.canLogIn())
			{
				...
				catch (InvalidCredentialsException e)
				{
					...
					else
					{
						if (user.contains("@"))
						{
							ErrorUtils.showClickableMessage("Invalid username or password!" + "<ul>" + "<li> Check your username and password. If you use OLD non-migrated account try using username instead of email." + "<li> You need to use  paid minecraft account credentials. Mojang account without paid MC won't work." + "</ul>", JOptionPane.ERROR_MESSAGE);
						}
						else
						{
							ErrorUtils.showClickableMessage("Invalid username or password. " + "<ul>" + "<li> Check your username and password. If you use new mojang account try using email instead of username." + "<li> You need to use  paid minecraft account credentials. Mojang account without paid MC won't work." + "<ul>", JOptionPane.ERROR_MESSAGE);
						}
						return null;
					}
				}
			...
			if (isValid(authentication))
			{
				...
				if ((authentication.isLoggedIn()) && (authentication.canPlayOnline()))
				{
					...
					if ((authentication instanceof YggdrasilUserAuthentication))
					{
						...
						return new LoginResponse(Integer.toString(authentication.getAgent().getVersion()), "token", displayName, authentication.getAuthenticatedToken(), authentication.getSelectedProfile().getId().toString(), authentication);
					}
				}
